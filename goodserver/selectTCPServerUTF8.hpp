...
public:
	...
	int initSocket()
	{
		...
		{
			std::cout << u8"初始化Winsock环境失败" << std::endl;
		}
		else
		{
			std::cout << u8"已成功初始化Winsock环境!" << std::endl;
		}
#endif 
		ssock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
		if (INVALID_SOCKET == ssock)
		{
			std::cout << u8"初始化服务器失败" << std::endl;
			return CMD_ERROR;
		}
		else
		{
			std::cout << u8"服务器初始化成功!" << std::endl;
		}
		return CMD_SUCCESS;
	}
	...
	int bindServer(const char* ip, unsigned short port)
	{
		if (INVALID_SOCKET == ssock)
		{
			std::cout << u8"套接字未初始化或无效" << std::endl;
			return CMD_ERROR;
		}
		...
		if (SOCKET_ERROR == res)
		{
			std::cout << u8"绑定端口失败" << std::endl;
			return CMD_ERROR;
		}
		else
		{
			std::cout << u8"端口绑定成功!" << std::endl;
		}

		if (SOCKET_ERROR == listen(ssock, 5))
		{
			std::cout << u8"侦听端口失败" << std::endl;
			return -1;
		}
		else
		{
			std::cout << u8"侦听端口成功!" << std::endl;
		}
	...
	bool OnRun()
	{
		...
		if (res < 0)
		{
			std::cout << u8"select模型未知错误，任务结束" << std::endl;
			terminal();
			return false;
		}
	...
	int sendMessage(SOCKET csock, PackType* msg)
	{
		if (INVALID_SOCKET == ssock)
		{
			std::cout << u8"服务器套接字未初始化或无效" << std::endl;
			return CMD_ERROR;
		}
		int res = send(csock, (const char*)msg, sizeof(PackType), 0);
		if (SOCKET_ERROR == res)
		{
			std::cout << u8"发送数据包失败" << std::endl;
			return CMD_ERROR;
		}
		else
		{
	...
	virtual void handleMessage(CLIENT* c, Pack* pk)
	{
		...
		case CMD_PRIVATEMESSAGE:
		{
			...
			{
				MessagePack pack1;
				strcpy(pack1.message, u8"私信发送失败，目标用户不存在或已离线");
		...
		case CMD_MESSAGE:
		{
			...
			std::cout << u8"从客户端(" << c->getSock() << u8")收到的消息 :CMD=" << pack->CMD << u8" LENGTH=" << pack->LENGTH << u8" DATA=" << pack->message << std::endl;
			strcpy(pack->message, u8"消息已成功被服务器接收!");
		...
		case CMD_NAME:
		{
			...
			if (it != clients.end())
			{
				...
				std::cout << u8"用户" << oldName << u8"(" << c->getSock() << u8")改名为" << userName << std::endl;
				userName = u8"已成功更改名称，现在的昵称为 " + userName;
				MessagePack pack1(userName.c_str());
				sendMessage(c->getSock(), &pack1);
			}
			else
			{
				MessagePack pack1(u8"重命名失败");
				sendMessage(c->getSock(), &pack1);
			}
			break;
		}
		...
		default:
		{
			std::cout << u8"无法解析的消息:CMD=" << pk->CMD << u8" length=" << pk->LENGTH << std::endl;
			break;
		}
		}
	}
...
private:
	...
	CLIENT* acceptClient()
	{
		...
		{
			std::cout << u8"无效的客户端套接字" << std::endl;
		}
		else
		{
			std::cout << u8"新客户端连接:" << csock << " IP:" << inet_ntoa(csin.sin_addr) << std::endl;
		}
	...
	int recvPack(CLIENT* c)
	{
		...
		if (len <= 0)
		{
			std::cout << u8"客户" << c->getUserName() << u8"(csock=" << csock << u8")已断开连接" << std::endl;
